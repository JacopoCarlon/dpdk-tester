...
# this needs to be updated betwen whiskey and treebeard

##  ##  ##  
#   ### --- RETIS treebeard-cplex3 config:
#   # Server B Access (treebeard)
#   SERVER_B_USER="jcarlon"
#   SERVER_B_HOST="cplex3"
#   SERVER_B_SCRIPT_DIR="/home/jcarlon/test/dpdk-tester/generator"
#   # Server A Configuration (cplex3)
...
### --- Server A Configuration (whiskey) --- --- 
...
run_latency_test() {
    ...
    ####    ####    # ---------- collect cstate&power : <turbostat + RAPL> ----------
    ####    ####    echo "[$(date +%T)] Starting RAPL (in background) and afterwards turbostat, and then wait for rapl completion."
    ####    ####    # Start RAPL in the background so it runs in parallel with turbostat
    ####    ####    $RAPL_SCRIPT -y -r -c $((MEASUREMENT_DURATION + 2)) -s 1 "$power_file" &
    ####    ####    RAPL_PID=$!
    ####    ####    ## TODO: TO_remember: turbostat needs : <sudo modprobe msr>
    ####    ####    # turbostat captures all available C-states during the measurement window
    ####    ####    sudo turbostat --quiet sleep $MEASUREMENT_DURATION > "$cstates_file"
    ####    ####    # Wait for RAPL to finish (it may already have ended)
    ####    ####    wait $RAPL_PID
    ####    ####    echo "[$(date +%T)] RAPL and turbostat finished."
    ####    ####    # ---- end of tests and of collections, kill cplex/lace nicely ----
